...
class BannerPreviewRenderer extends StandardContentPreviewRenderer
{
    public function renderPageModulePreviewContent(GridColumnItem $item): string
    {
        ...
        if ($recordObj->has('CType') && $recordObj->get('CType') === 'gsb_banner') {
            if ($recordObj->has('tx_link_text')) {
                $content .= $this->linkEditContent('<div class="text-left">' . $recordObj->get('tx_link_text') . '</div>', $record);
            }
